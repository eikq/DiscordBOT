...
import type { SelfKnowledgeSnapshot } from '../../intelligence';
...
function DevicesPage(props: Props) {
  ...
  return (
    <div className="jai-page">
      ...
      {!devices.length ? <EmptyState title="No device providers attached" detail="Missing hardware is unavailable, never silently successful." /> : <div className="jai-device-grid">{devices.map(device => <article key={device.id} className="jai-device-card"><div><Laptop size={22} /><span className={`jai-badge ${device.simulated ? 'jai-badge--simulation' : 'jai-badge--neutral'}`}>{device.simulated ? 'Simulation' : device.status}</span></div><h3>{device.label}</h3><p>{device.kind}</p><div className="jai-access-row"><span className={device.simulated ? '' : 'is-granted'}>{device.simulated ? 'View simulation' : 'View'}</span><span>Control</span><span>Configure</span><span>Admin</span></div><small>{device.simulated ? 'Fixture only · no live hardware effect' : device.node}</small><ExpertDetails summary="Distribution metadata"><p>{device.distribution.join(' · ')}</p></ExpertDetails></article>)}</div>}
      <SectionCard title="Home CCTV connection" description="Owner-only provider contract · no live camera is claimed" tone="amber">
        <div className="jai-task-meta"><span><small>Current state</small><strong>{cctvConnect?.status || 'UNKNOWN'}</strong></span><span><small>Provider</small><strong>{cctvConnect?.requirements.providers.join(' · ') || 'RTSP / ONVIF / vendor NVR adapter needed'}</strong></span><span><small>Local acceptance</small><strong>{cctvConnect?.localAcceptance || 'BLOCKED_LOCAL_ACCEPTANCE'}</strong></span></div>
        <p className="jai-muted">To connect later: identify the camera/NVR vendor and model, provide a known LAN address and supported protocol, then store credentials through a local secret reference. No broad LAN scan or public exposure is authorized.</p>
        <div className="jai-access-row"><span className="is-granted">Discover contract</span><span>Connect</span><span>View</span><span>Control</span></div>
        <ExpertDetails summary="CCTV contract evidence"><p>{cctvConnect?.reason || 'No provider evidence is available.'}</p><p>{cctvConnect?.evidence.join(' · ') || 'No runtime evidence.'}</p></ExpertDetails>
      </SectionCard>
...
function SystemPage(props: Props) {
  ...
  const intelligence = props.status?.selfKnowledge?.capabilities ?? [];
  const filteredIntelligence = useMemo(() => intelligence.filter(item => `${item.id} ${item.displayName} ${item.description} ${item.status} ${item.reason}`.toLowerCase().includes(capabilityQuery.toLowerCase())), [intelligence, capabilityQuery]);
  const capabilityRows = intelligence.length ? filteredIntelligence : filtered;
  const statusCounts = intelligence.reduce<Record<string, number>>((counts, item) => ({ ...counts, [item.status]: (counts[item.status] ?? 0) + 1 }), {});
  const goals = props.status?.selfKnowledge?.goals ?? [];
  return (
    <div className="jai-page">
      ...
      {intelligence.length ? <div className="jai-status-grid jai-status-grid--four"><Metric label="Available now" value={String(statusCounts.AVAILABLE ?? 0)} detail="runtime evidence" /><Metric label="Needs setup" value={String((statusCounts.NEEDS_CONFIGURATION ?? 0) + (statusCounts.NEEDS_PROVIDER ?? 0) + (statusCounts.NEEDS_DEPENDENCY ?? 0))} detail="configuration or provider" /><Metric label="Simulation" value={String(statusCounts.SIMULATION ?? 0)} detail="no live effect" /><Metric label="Local acceptance" value={String(intelligence.filter(item => item.localAcceptance === 'BLOCKED_LOCAL_ACCEPTANCE').length)} detail="owner machine required" /></div> : null}
      <SectionCard title="End-to-end goals" description="What Jarvis can complete through declared capability routes" tone="violet">
        {!goals.length ? <EmptyState title="Goal evidence unavailable" detail="No authoritative Goal Catalog snapshot was returned." /> : <div className="jai-capability-table" role="table"><div className="jai-capability-table__head" role="row"><span>Goal</span><span>Readiness</span><span>Scope</span><span>Requirement</span><span>Evidence</span></div>{goals.map(goal => <div className="jai-capability-table__row" role="row" key={goal.id}><span><strong>{goal.name}</strong><small>{goal.description}</small></span><span><span className={`jai-badge ${goal.status === 'CAN_DO_NOW' ? 'jai-badge--success' : 'jai-badge--neutral'}`}>{goal.status.replaceAll('_', ' ')}</span><small>{goal.reason}</small></span><span>{goal.scope.replaceAll('_', ' ')}</span><span>{goal.permissionImplications}</span><span>{goal.verificationExpectation}<ExpertDetails summary="Goal evidence"><code>{goal.id}</code><p>{goal.capabilityIds.join(' · ') || 'structured self knowledge'}</p><p>{goal.evidence.join(' · ')}</p></ExpertDetails></span></div>)}</div>}
      </SectionCard>
      <SectionCard title="Capability Explorer" description="Registered contracts are not proof of live provider health" tone="cyan" action={<label className="jai-inline-search"><Search size={15} /><input value={capabilityQuery} onChange={event => setCapabilityQuery(event.target.value)} placeholder="Search capabilities" /></label>}>
        {!capabilityRows.length ? <EmptyState title="No capabilities match" detail="Self Knowledge returned no matching evidence." /> : intelligence.length ? <div className="jai-capability-table" role="table"><div className="jai-capability-table__head" role="row"><span>Name</span><span>Current state</span><span>Risk / permission</span><span>Requirements</span><span>Evidence</span></div>{filteredIntelligence.map(item => <div className="jai-capability-table__row" role="row" key={item.id}><span><strong>{item.displayName}</strong><small>{item.description}</small></span><span><span className={`jai-badge ${item.status === 'SIMULATION' ? 'jai-badge--simulation' : item.status === 'AVAILABLE' ? 'jai-badge--success' : 'jai-badge--neutral'}`}>{item.status.replaceAll('_', ' ')}</span><small>{item.reason}</small></span><span>{item.sideEffect === 'read' ? 'Read only' : 'Mutation'}<small>{item.permission.class.replaceAll('_', ' ')}</small></span><span>{item.requirements.ownerInput[0] || item.requirements.providers[0] || item.requirements.dependencies[0] || 'No additional requirement recorded'}<small>{item.localAcceptance.replaceAll('_', ' ')}</small></span><span>{item.competence?.confidence === null || item.competence?.confidence === undefined ? 'Competence unknown' : `${Math.round(item.competence.confidence * 100)}% verified confidence`}<small>{item.support.verification === 'registered' ? 'Deterministic verification registered' : 'No registered postcondition'}</small><ExpertDetails summary="Technical evidence"><code>{item.id}</code><p>{item.implementation.maturity} · {item.implementation.mode} · registered={String(item.registered)}</p><p>{item.provider.kind} · {item.provider.requiredService}</p><p>{item.distribution.join(' · ')}</p><p>{item.evidence.join(' · ')}</p></ExpertDetails></span></div>)}</div> : <div className="jai-capability-table" role="table"><div className="jai-capability-table__head" role="row"><span>Name</span><span>Category</span><span>Risk / permission</span><span>Provider</span><span>Status</span></div>{filtered.map(item => <div className="jai-capability-table__row" role="row" key={item.id}><span><strong>{humanCapability(item.id)}</strong></span><span>{capabilityCategory(item.id)}</span><span>{item.sideEffect === 'read' ? 'Read only' : 'Policy evaluated'}</span><span>{item.providerKind}<small>{item.requiredService}</small></span><span><span className="jai-badge jai-badge--neutral">Registered</span><ExpertDetails summary="Capability ID"><code>{item.id}</code></ExpertDetails></span></div>)}</div>}
      </SectionCard>
