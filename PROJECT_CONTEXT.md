...
Standalone Jarvis now uses llama.cpp OpenAI-compatible `qwen38-cyber` at `http://127.0.0.1:8086/v1` (32768 context) as its canonical local model. Discord still uses Ollama `LLM_BASE_URL` / `LLM_MODEL`. Conversation history and durable memory stay SQLite-canonical (schema v3) with a rebuildable Obsidian view at `data/jarvis/obsidian/`. Permission-first outcomes, `BUILD_WEBSITE` / `BUILD_SOFTWARE` planning, persistent/revalidated THIS_GOAL leases (`data/jarvis/runtime/permissions.db`), and typed ProjectWorkspace install/build/test/localhost-preview landed on `local/jarvis-persistent-permission-builder-runtime-v1-2026-08-22`. See `docs/JARVIS_QWEN_MEMORY_BUILD_STUDIO.md`. Persistence is evidence, never authority. CLICK / TYPE / SUBMIT remain `PREPARE_CONTRACT`. Live todo-website apply/preview was not claimed in the builder-runtime session.
