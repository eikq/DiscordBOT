...
A Jarvis-first addendum now takes priority: finish standalone Jarvis Core v1 before more Discord work (`JARVIS_FIRST_DIRECTIVE.md`, `JARVIS_FIRST_TASKS.md`). Contracts for JARVIS-001/002/003 exist in `src/jarvis/`. JARVIS-004/005 Discord adapters exist in the working tree and are kept, but further Discord integration is deferred. Standalone text path: `npm run jarvis:ask`. Isolated lab UI: `/jarvis-lab` (JF-006/JF-007 + UI-R1–R8 command center + JF-008 mic/STT + JF-008B latency/STT hardening + JF-009 optional Edge-TTS speech + JF-010 actions + JF-011 runtime/service status + JF-012 reminders/scheduler + JF-013 read-only public web research + JF-013.5 natural intent resolution + JF-014 read-only local workspace intelligence + JF-014.5 privilege leases + fail-closed PRIVATE_BROWSER/Whonix policy + JF-014.6 operations event bus with SSE replay + JF-015 work agent with CapabilityHost invocation, isolated `work.db`, and typed `/api/jarvis/ask` routing + EVO-001–010 evolution lifecycle with isolated `evolution.db` and one experience per task + JF-016/017/018 simulated vision/monitor/devices + research-addendum operational traces in isolated `ops.db`, versioned RuntimeSpec, model trust tiers, fixture-only certification, spec optimizer with no auto-promote, simulated artifact/media contracts; does not replace the Digital Me dashboard). Local hardware/browser/Ollama acceptance is queued in `BLOCKED_LOCAL_ACCEPTANCE.md` (LA-001–LA-027). **LA-001** and **LA-002** are **PARTIAL** (owner 2026-08-20); do not mark LIVE_VERIFIED. **LA-026** Presenter is **PARTIAL** (software: structured facts, TTS-driven segments, repeat/back — CLOUD_VERIFIED unit; live speech↔motion NEEDS_LOCAL_VERIFY). **LA-027** is **PARTIAL — NATIVE_SHELL_REQUIRED** (real display enum locally; intersection matching CLOUD_VERIFIED unit; browser host cannot move Chrome/Edge). ADR-023 Phase 1 contracts are implemented as cloud-safe scaffolding; the native helper is **not installed**. Cloud continuation branch: `cursor/jarvis-cloud-evolution-2026-08-20`. Queue pointer: `CURSOR_CLOUD_QUEUE_STATUS.md`. Handoff: `CURSOR_CLOUD_PRESENTER_DESKTOP_HANDOFF.md`. JF-013 Research Intelligence V2 (Queue 02) is IMPLEMENTED + CLOUD_VERIFIED (unit): structured query plans, source trustClass, claim/evidence links, contradiction reporting, cache honesty, citation grounding. Canonical Memory Intelligence V2 (Queue 03 / JF-004C) is IMPLEMENTED + CLOUD_VERIFIED (unit): SQLite remains canonical; hybrid fusion drops orphan vectors; query-aware classes; episode candidates; owner correction; optional Presenter provenance. Queue 04 Procedural Skills V2 extends fail-closed evolution: one task lifecycle, reviewable skill candidates, trusted-only retrieval, structured failure knowledge, isolated benchmarks, `autoPromote=false`, no self-approval. Queue 05 Model Registry V2 routes by evidence, trust tier, certification, hardware idle, and latency; RESTRICTED models are never auto-selected and never security authorities; Cloud certification stays FIXTURE_ONLY (never LIVE_VERIFIED); no model downloads or invented hardware benchmarks. Queue 06 Realtime Voice Interaction is IMPLEMENTED + UNIT_VERIFIED (mocks): explicit turn states, barge-in classification, one Presenter playback clock, NightCycle pauses for `realtime_voice`, mutating WorkAgent apply is not blindly cancelled; persona stays independent from voice. Do not claim live microphone/STT/TTS/RVC. Queue 07 Perception / screen / CCTV / devices is IMPLEMENTED + UNIT_VERIFIED (mocks): SEE != CLICK, VIEW != CONTROL, untrusted vision, default CCTV view/search only, bounded perceptual memory candidates, anomaly pipeline never auto-acts. Do not claim live camera or device control. Queue 08 Proactive Runtime is IMPLEMENTED + UNIT_VERIFIED: five-level resource priority, coordinator over existing reminders/monitor/NightCycle (no fourth scheduler), notice-only suggestions, Night V2 mapped pipeline with yield/resume and `autoPromoted: false`. Do not claim live GPU sensors or unattended Night coding. Queue 09 Command Center V2 is IMPLEMENTED + UNIT_VERIFIED: `/jarvis-lab` shows one operational mode at a time (Assistant, Presenter, Operations, Memory, Intelligence, Devices) over existing snapshots; global REAL/SIMULATION/DEGRADED/OFFLINE; simulation is never live hardware; no backend rewrite. Do not claim live lab visual QA. Queue 10 Security Hardening is IMPLEMENTED + UNIT_VERIFIED: untrusted research/model/vision cannot become authority; SSRF blocks IPv4-mapped IPv6 and metadata; native helper IPC fails closed; traces omit secrets; gitignore/Night denylist cover runtime DBs, display aliases, and model weights. Queue 11 Full-System Cloud Integration is IMPLEMENTED + CLOUD_VERIFIED (unit): fourteen offline fixtures; independent `turnId` vs `requestId`; distinct failure codes (`UNSUPPORTED_HOST`, `PERMISSION_REQUIRED`, `DENIED`, `TIMEOUT`, `PROVIDER_UNAVAILABLE`, `INSUFFICIENT_DATA`, `UNKNOWN_DISPLAY`, `VERIFICATION_FAILED`) are not collapsed. Simulation is never LIVE. Queue 12 Final Cloud Audit is a physical-machine handoff only (`CURSOR_LOCAL_ACCEPTANCE_NEXT.md`, ADR-034). Cloud feature queues stop here. Do not mark LIVE_VERIFIED. Qdrant is **not** started and is not a source of truth. PRIVATE_BROWSER stays fail-closed. Whonix/Tor is LOCAL_VERIFY_REQUIRED. Do not start LA-003+. BitLocker/Device Encryption must not be enabled by Jarvis. Platform queue: `JARVIS_PLATFORM_TASKS.md`.
